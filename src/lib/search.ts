...
import type { Message } from './community';

export type SearchResultType = '角色' | '武器' | '地图' | '地点' | '攻略' | '世界观' | '留言';
...
function readMessages(): Message[] {
  try {
    const file = path.join(process.cwd(), 'src/data/community/messages.json');
...
export function searchAll(query: string, limit?: number): SearchResult[] {
  ...
  for (const m of messages) {
    const match = multiFieldFuzzyMatch(q, [
      { text: m.name, weight: 1.2 },
      { text: m.content, weight: 1.0 },
      { text: m.tag, weight: 0.6 },
    ]);
    if (match) {
      results.push({
        id: m.id,
