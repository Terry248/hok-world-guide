...
import { Search, User, Sword, Map, MapPin, FileText, Globe, MessageSquare } from 'lucide-react';
...
const typeIcons: Record<string, React.ReactNode> = {
  ...
  '留言': <MessageSquare className="w-4 h-4" />,
};
...
const typeColors: Record<string, string> = {
  ...
  '留言': 'bg-pink-500/20 text-pink-400 border-pink-500/30',
};
    ...
    <div className="space-y-6">
      ...
      {!initialQuery && (
        <div className="text-center py-12 text-text-muted">
          ...
          <p>输入关键词开始搜索</p>
          <p className="text-sm mt-1 opacity-60">支持搜索角色名、武器、地图区域、攻略标题、留言内容等</p>
        </div>
      )}
